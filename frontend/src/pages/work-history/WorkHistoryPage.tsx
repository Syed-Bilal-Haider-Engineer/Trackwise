...
export function WorkHistoryPage() {
  ...
  // Appointments — filtered by month
  ...
  // Header subtitle changes per tab
  const headerSub = {
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {activeTab === 'work' && (
          filteredEntries.length === 0 ? (
            ...
            filteredEntries.map(entry => (
              <EntryCard key={entry.id} entry={entry} onDelete={handleDeleteWork} />
            ))
          )
        )}
        ...
        {activeTab === 'appointments' && (
          filteredAppts.length === 0 ? (
            ...
            filteredAppts.map(appt => (
              <AppointmentCard key={appt.id} appt={appt} onDelete={handleDeleteAppt} />
            ))
          )
        )}
